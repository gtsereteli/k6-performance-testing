...
export default function () {
    ...
    const crocNameUpdate = 'newName' + Date.now();
    const patchPrivateCrocRes = http.patch(
    ...
    check(patchPrivateCrocRes, {
        'is private PATCH response status 200': (r) => r.status === 200,
        'is updated name correct': (r) => r.json().name === crocNameUpdate,
        'is PATCH crocodile id correct': (r) => r.json().id === crocodileId,
    });
}
